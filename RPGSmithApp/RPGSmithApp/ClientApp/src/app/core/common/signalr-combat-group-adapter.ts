...
export class SignalRCombatGroupAdapter extends ChatAdapter implements IChatGroupAdapter {
  ...
  private initializeConnection(): void {
    this.hubConnection = new signalR.HubConnectionBuilder()
      .withUrl(`${SignalRCombatGroupAdapter.serverBaseUrl}groupchat`)
      .build();
  ...
  listFriends(): Observable<ParticipantResponse[]> {
    ...
    //}    
    return this.http
      .post(`${SignalRCombatGroupAdapter.serverBaseUrl}api/chat/listFriends`, { currentUserId: this.userId }, this.getRequestHeaders())
      .pipe(
  ...
  getMessageHistory(participant: any): Observable<Message[]> {
    ...
    //return of([]);
    return this.http
      .post(`${SignalRCombatGroupAdapter.serverBaseUrl}api/chat/getChatHistory?currentUserID=${this.userId}`, JSON.stringify(participant), this.getRequestHeaders())
      .pipe(
        map((res: any) => res),
        catchError((error: any) => Observable.throw(error || 'Server error'))
      );
