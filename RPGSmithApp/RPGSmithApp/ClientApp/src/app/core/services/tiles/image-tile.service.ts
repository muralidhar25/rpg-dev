...
@Injectable()
export class ImageTileService extends EndpointFactory {
  ...
  createImageTile<T>(model: CharacterTile): Observable<T> {

    let endpoint = this.createApi;
    if (model.imageTile.imageTileId > 0)
      endpoint = this.updateApi;

    return this.http.post<T>(endpoint, JSON.stringify(model), this.getRequestHeaders())
      .catch(error => {
        return this.handleError(error, () => this.createImageTile(model));
      });
  }
  ...
  public imageTileModelData(model: any, characterId: number, pageId: number, view: string, pageDefaultData: CharacterDashboardPage): any {
    ...
    if (view == VIEW.EDIT) {
      ...
      modelData = {
        ...
        imageTile: {
          imageTileId: model.imageTileId ? model.imageTileId : 0,
          characterTileId: model.characterTileId ? model.characterTileId : 0,
  ...
  createRulesetImageTile<T>(model: RulesetTile): Observable<T> {
    ...
    return this.http.post<T>(endpoint, JSON.stringify(model), this.getRequestHeaders())
      .catch(error => {
        return this.handleError(error, () => this.createRulesetImageTile(model));
      });
  }
  ...
  public imageTileRulesetModelData(model: any, rulesetId: number, pageId: number, view: string, pageDefaultData: RulesetDashboardPage): any {
