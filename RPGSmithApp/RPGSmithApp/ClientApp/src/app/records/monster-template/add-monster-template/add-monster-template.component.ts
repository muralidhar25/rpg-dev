...
@Component({
  selector: 'app-add-monster-template',
  templateUrl: './add-monster-template.component.html',
  styleUrls: ['./add-monster-template.component.scss']
})
export class AddMonsterTemplateComponent implements OnInit {
  ...
  submitForm() {
    if (this.itemsList.filter(x => x.selected).length) {
      this.sharedService.updateAddMonsterTemplatesList(this.itemsList.filter(x => x.selected));
      this.close();
    }
    else {
  ...
  setMonsterTemplate(event: any, monsterTemplate: any) {
    this.itemsList.map((item) => {
      if (item.monsterTemplateId == monsterTemplate.monsterTemplateId) {
