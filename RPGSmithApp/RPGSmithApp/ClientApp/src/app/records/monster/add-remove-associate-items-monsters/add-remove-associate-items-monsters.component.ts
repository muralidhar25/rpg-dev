...
@Component({
  selector: 'app-add-remove-associate-items-monsters',
  templateUrl: './add-remove-associate-items-monsters.component.html',
  styleUrls: ['./add-remove-associate-items-monsters.component.scss']
})
export class AddRemoveAssociateMonstersComponent implements OnInit {
  ...
  private initialize() {
    this.itemsList.map((x) => {
      ...
      if (this.selectedItems.filter(a => a.monsterTemplateId == x.monsterTemplateId).length) {
        x.selected = true;
  ...
  submitForm() {
    ...
    debugger
      ...
      this.isLoading = true;
    let _msg = 'Updating associated Monsters ...';
    this.alertService.startLoadingMessage("", _msg);
    debugger
    this.monsterTemplateService.AddRemoveMonsterRecords(this.itemsList, this.monster.monsterId,'M')
        .subscribe(data => {
          //if (data) {
