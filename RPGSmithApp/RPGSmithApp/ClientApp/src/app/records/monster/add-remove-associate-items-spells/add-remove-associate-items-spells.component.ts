...
@Component({
  selector: 'app-add-remove-associate-items-spells',
  templateUrl: './add-remove-associate-items-spells.component.html',
  styleUrls: ['./add-remove-associate-items-spells.component.scss']
})
export class AddRemoveAssociateSpellsComponent implements OnInit {
  ...
  private initialize() {
    this.itemsList.map((x) => {
      ...
      if (this.selectedItems.filter(a => a.spellId == x.spellId).length) {
        x.selected = true;
  ...
  submitForm() {
    ...
    debugger
      ...
      this.isLoading = true;
      let _msg = 'Updating associated Spells ...';
    this.alertService.startLoadingMessage("", _msg);
    debugger
    this.monsterTemplateService.AddRemoveMonsterRecords(this.itemsList, this.monster.monsterId,'S')
        .subscribe(data => {
          //if (data) {
