...
export class EndpointFactory {
  ...
  private get loginUrl() { return this.configurations.baseUrl + this.configurations.loginApi; }
  ...
  loginSocialPlatform<T>(token: string, grantType: string): Observable<T> {
  ...
  protected getRequestHeadersWithResponse(responseType: string): { headers: HttpHeaders | { [header: string]: string | string[]; }, responseType: string } {
    let headers;
    if (Utilities.IsIEorEdge) {
      headers = new HttpHeaders({
        'Authorization': 'Bearer ' + this.authService.accessToken,
    ...
    return { headers: headers, responseType: responseType };
  }

  protected getRequestHeadersNew(): HttpHeaders | { [header: string]: string | string[]; } {
    let headers;
    if (Utilities.IsIEorEdge) {
  ...
  protected getRequestFileHeaders(): { headers: HttpHeaders | { [header: string]: string | string[]; } } {
    let headers;
    if (Utilities.IsIEorEdge) {
      headers = new HttpHeaders({
        'Authorization': 'Bearer ' + this.authService.accessToken,
        //'Content-Type': 'multipart/form-data; boundary=----WebKitFormBoundary7MA4YWxkTrZu0gW', //'application/json',
        'Accept': `application/vnd.iman.v${EndpointFactory.apiVersion}+json, application/json, text/plain, */*`,
        'App-Version': ConfigurationService.appVersion,
        'Cache-Control': 'no-cache',
        'Pragma': 'no-cache',
