        ...
        public async Task<IActionResult> Duplicate([FromBody] RulesetDashboardLayout model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    int old_RulesetDashboardLayoutId = layout.RulesetDashboardLayoutId; ;
                    if (_RulesetDashboardPages.Any())
                    {
                        old_RulesetDashboardLayoutId = (int)_RulesetDashboardPages.FirstOrDefault().RulesetDashboardLayoutId;
                    }

                    _rulesetDashboardLayoutService.Create_sp(layout, GetUserId(), old_RulesetDashboardLayoutId);
                    ...
                    if (_RulesetDashboardPages.Where(x => x.RulesetDashboardPageId == defaultPageIdOfOldLayout).Any())
                    {
                        string old_DefaultPageName = _RulesetDashboardPages.Where(x => x.RulesetDashboardPageId == defaultPageIdOfOldLayout).FirstOrDefault().Name;

                        _rulesetDashboardLayoutService.UpdateDefaultPageId(layout.RulesetDashboardLayoutId, old_DefaultPageName);
                    }
